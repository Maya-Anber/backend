<?php
// backend/exchange/update_request.php
...
$response_message = sanitize_text($input['message'] ?? '');
...
if ($action === 'approve') {
    ...
    try {
        $stmt = $conn->prepare("UPDATE exchange_requests SET status='approved', response_message=?, response_date=NOW() WHERE request_id=?");
    ...
    $stmt = $conn->prepare("UPDATE exchange_requests SET status='rejected', response_message=?, response_date=NOW() WHERE request_id=?");
    $stmt->bind_param('si', $response_message, $request_id);
    $stmt->execute();
    ...
    $stmt = $conn->prepare("UPDATE exchange_requests SET status='cancelled', response_message=?, response_date=NOW() WHERE request_id=?");
    $stmt->bind_param('si', $response_message, $request_id);
    $stmt->execute();
    json_response(['success'=>true,'message'=>'Request cancelled']);

} elseif ($action === 'complete') {
    // Either party can mark complete (you can require both in future)
    ...
    try {
        ...
        // Optional: archive (copy row to archive table)
        // Note: Adjust column names to match your archive schema exactly.
        $stmt = $conn->prepare("INSERT INTO exchange_requests_archive (request_id, requester_id, owner_id, requested_listing_id, offered_listing_id, status, request_message, response_message, request_date, response_date, archived_date) SELECT request_id, requester_id, owner_id, requested_listing_id, offered_listing_id, status, request_message, response_message, request_date, response_date, NOW() FROM exchange_requests WHERE request_id=?");
        $stmt->bind_param('i', $request_id);
        $stmt->execute();
